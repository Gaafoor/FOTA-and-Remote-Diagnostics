...
Std_ReturnType Fls_Write(Fls_AddressType TargetAddress, const uint8* SourceAddressPtr, Fls_LengthType Length)
{
    ...
    /*if (hal_status != HAL_OK)
    {
        ...
        Det_ReportError(FLASH_DRIVER_ID, FLS_INSTANCE_ID, FLS_WRITE_API_ID, FLS_E_HAL_ERROR);
        return E_NOT_OK;
    }*/

    // Program Flash
    for (uint32_t i = 0; i < Length; i += 4) // Assuming flash programming is done in 32-bit words
    {
        hal_status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, TargetAddress + i, *(uint32_t*)(SourceAddressPtr + i));
